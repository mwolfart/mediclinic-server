...
func (r *roleR) GetUseridGenericusers() GenericuserSlice {
	if r == nil {
		return nil
	}
...
var (
	roleAllColumns            = []string{"id", "description"}
	roleColumnsWithoutDefault = []string{"description"}
...
func (o *Role) UseridGenericusers(mods ...qm.QueryMod) genericuserQuery {
	...
	queryMods = append(queryMods,
		qm.InnerJoin("\"rolesonuser\" on \"genericuser\".\"id\" = \"rolesonuser\".\"userid\""),
		qm.Where("\"rolesonuser\".\"roleid\"=?", o.ID),
	)
...
func (roleL) LoadUseridGenericusers(ctx context.Context, e boil.ContextExecutor, singular bool, maybeRole interface{}, mods queries.Applicator) error {
	var slice []*Role
	var object *Role
	...
	query := NewQuery(
		qm.Select("\"genericuser\".\"id\", \"genericuser\".\"name\", \"genericuser\".\"email\", \"genericuser\".\"age\", \"genericuser\".\"gender\", \"genericuser\".\"passwordhash\", \"a\".\"roleid\""),
		qm.From("\"genericuser\""),
		qm.InnerJoin("\"rolesonuser\" as \"a\" on \"genericuser\".\"id\" = \"a\".\"userid\""),
		qm.WhereIn("\"a\".\"roleid\" in ?", argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if singular {
		...
		for _, foreign := range resultSlice {
			...
			foreign.R.RoleidRoles = append(foreign.R.RoleidRoles, object)
		}
		return nil
	}
	...
	for i, foreign := range resultSlice {
		...
		for _, local := range slice {
			if local.ID == localJoinCol {
				...
				foreign.R.RoleidRoles = append(foreign.R.RoleidRoles, local)
				break
			}
		}
	}
...
// Sets related.R.RoleidRoles appropriately.
func (o *Role) AddUseridGenericusers(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Genericuser) error {
	var err error
	...
	for _, rel := range related {
		query := "insert into \"rolesonuser\" (\"roleid\", \"userid\") values ($1, $2)"
	...
	for _, rel := range related {
		if rel.R == nil {
			rel.R = &genericuserR{
				RoleidRoles: RoleSlice{o},
			}
		} else {
			rel.R.RoleidRoles = append(rel.R.RoleidRoles, o)
		}
	}
	return nil
}
...
// Sets o.R.RoleidRoles's UseridGenericusers accordingly.
// Replaces o.R.UseridGenericusers with related.
// Sets related.R.RoleidRoles's UseridGenericusers accordingly.
func (o *Role) SetUseridGenericusers(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Genericuser) error {
	query := "delete from \"rolesonuser\" where \"roleid\" = $1"
	...
	removeUseridGenericusersFromRoleidRolesSlice(o, related)
	if o.R != nil {
...
// Sets related.R.RoleidRoles.
func (o *Role) RemoveUseridGenericusers(ctx context.Context, exec boil.ContextExecutor, related ...*Genericuser) error {
	if len(related) == 0 {
		return nil
	}
	...
	var err error
	query := fmt.Sprintf(
		"delete from \"rolesonuser\" where \"roleid\" = $1 and \"userid\" in (%s)",
...
func removeUseridGenericusersFromRoleidRolesSlice(o *Role, related []*Genericuser) {
	for _, rel := range related {
		if rel.R == nil {
			continue
		}
		for i, ri := range rel.R.RoleidRoles {
			if o.ID != ri.ID {
				continue
			}

			ln := len(rel.R.RoleidRoles)
			if ln > 1 && i < ln-1 {
				rel.R.RoleidRoles[i] = rel.R.RoleidRoles[ln-1]
			}
			rel.R.RoleidRoles = rel.R.RoleidRoles[:ln-1]
			break
		}
	}
}
