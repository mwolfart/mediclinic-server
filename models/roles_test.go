...
func testRoleToManyUseridGenericusers(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	_, err = tx.Exec("insert into \"rolesonuser\" (\"roleid\", \"userid\") values ($1, $2)", a.ID, b.ID)
	if err != nil {
		t.Fatal(err)
	}
	_, err = tx.Exec("insert into \"rolesonuser\" (\"roleid\", \"userid\") values ($1, $2)", a.ID, c.ID)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := RoleSlice{&a}
	if err = a.L.LoadUseridGenericusers(ctx, tx, false, (*[]*Role)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testRoleToManyAddOpUseridGenericusers(t *testing.T) {
	var err error
	...
	for i, x := range foreignersSplitByInsertion {
		...
		if first.R.RoleidRoles[0] != &a {
			t.Error("relationship was not added properly to the slice")
		}
		if second.R.RoleidRoles[0] != &a {
			t.Error("relationship was not added properly to the slice")
		}
...
func testRoleToManySetOpUseridGenericusers(t *testing.T) {
	var err error
	...
	//
	// if len(b.R.RoleidRoles) != 0 {
	// 	t.Error("relationship was not removed properly from the slice")
	// }
	// if len(c.R.RoleidRoles) != 0 {
	// 	t.Error("relationship was not removed properly from the slice")
	// }
	if d.R.RoleidRoles[0] != &a {
		t.Error("relationship was not added properly to the slice")
	}
	if e.R.RoleidRoles[0] != &a {
		t.Error("relationship was not added properly to the slice")
	}
...
func testRoleToManyRemoveOpUseridGenericusers(t *testing.T) {
	var err error
...
var (
	roleDBTypes = map[string]string{`ID`: `integer`, `Description`: `character varying`}
	_           = bytes.MinRead
)
